...
A continuación, se presentan las épicas y User Stories definidos para el desarrollo de NodeSecure. Los criterios de aceptación han sido redactados utilizando el formato Gherkin (Given-When-Then) para asegurar su correcta validación en la fase de pruebas.

| Epic / Story ID | Título | Descripción | Criterios de Aceptación | Relacionado con (Epic ID) |
|---|---|---|---|---|
| **US01** | Visualizar red de almacenes | Como dueño de PYME, quiero visualizar mis almacenes registrados para tener un control general de mis sedes desde el dashboard de NodeSecure. | **Given** el usuario tiene almacenes registrados, **When** accede al sistema, **Then** el dashboard muestra la lista de almacenes asociados.<br>**Given** un usuario nuevo, **When** accede al panel, **Then** el sistema muestra un estado vacío invitando a registrar su primera sede. | EP01 |
| **US02** | Monitorear estado de conexión IoT | Como jefe de seguridad, quiero consultar el estado de conexión de los sensores magnéticos (Online/Offline) para evitar puntos ciegos en la vigilancia. | **Given** existen sensores registrados, **When** el usuario accede a la vista de dispositivos, **Then** el sistema muestra el estado de conexión actual de cada ESP32.<br>**Given** un sensor pierde conexión, **When** ocurre el timeout, **Then** el sistema lo marca como Offline. | EP01, EP08 |
| **US03** | Auditoría de discrepancias en tiempo real | Como jefe de operaciones, quiero monitorear la detección de mermas en tiempo real para intervenir rápidamente si ocurre una salida no registrada. | **Given** el motor de NodeSecure cruza datos, **When** detecta una apertura sin registro de inventario, **Then** genera una alerta de discrepancia en el dashboard.<br>**Given** una alerta generada, **When** el usuario está en línea, **Then** la visualiza sin necesidad de recargar la página. | EP01, EP03 |
| **US04** | Registrar nueva infraestructura | Como dueño de PYME, quiero registrar un nuevo almacén y sus zonas para empezar a monitorearlos con el hardware IoT. | **Given** el usuario ingresa datos válidos, **When** registra un almacén, **Then** el sistema lo guarda y habilita la vinculación de sensores.<br>**Given** datos incompletos, **When** intenta registrar, **Then** el sistema bloquea la acción indicando los campos faltantes. | EP02 |
| **US05** | Configurar turnos operativos | Como jefe de seguridad, quiero definir los horarios de apertura autorizados para que el sistema sepa cuándo una apertura física es sospechosa. | **Given** el usuario define un turno (ej. 8 AM - 6 PM), **When** guarda la configuración, **Then** NodeSecure ajusta las reglas de alerta de esa zona.<br>**Given** un acceso físico fuera del turno, **When** el sensor lo detecta, **Then** se clasifica automáticamente como Alerta Crítica. | EP02, EP03 |
| **US06** | Notificaciones Push de seguridad | Como jefe de seguridad, quiero recibir notificaciones Push en mi celular ante aperturas no autorizadas para reaccionar al instante. | **Given** ocurre una alerta crítica, **When** el sistema la procesa, **Then** NodeSecure envía una notificación Push al dispositivo del usuario.<br>**Given** permisos denegados en el navegador, **When** se intenta enviar, **Then** el sistema recurre al envío por Email. | EP03 |
| **US07** | Justificar / Descartar falsas alarmas | Como jefe de operaciones, quiero poder marcar una alerta como "Falsa Alarma" si el movimiento de stock fue autorizado verbalmente. | **Given** existe una alerta activa, **When** el usuario selecciona "Marcar como Falsa Alarma", **Then** el sistema requiere un comentario de justificación.<br>**Given** la justificación guardada, **When** se revisa la bitácora, **Then** el evento cambia a estado "Conciliado". | EP03, EP04 |
| **US08** | Bitácora de trazabilidad inalterable | Como auditor, quiero consultar un historial detallado de eventos físicos y lógicos para tener un índice exacto (Timestamps) al revisar el CCTV. | **Given** existen eventos físicos registrados, **When** el usuario abre la Trazabilidad, **Then** NodeSecure muestra la lista cronológica con timestamps exactos.<br>**Given** el usuario intenta borrar un evento, **When** hace clic, **Then** el sistema le deniega la acción por seguridad. | EP04 |
| **TS09** | Recepción de payloads IoT (API) | Como developer, quiero exponer un endpoint en ASP.NET Core que reciba los eventos del ESP32 para procesar las aperturas físicas. | **Given** un payload JSON válido (MAC, estado, timestamp), **When** el ESP32 hace POST al endpoint, **Then** la API guarda el evento físico.<br>**Given** un payload malformado, **When** llega a la API, **Then** el sistema retorna HTTP 400 Bad Request. | EP05, EP08 |
| **US10** | Filtros avanzados de auditoría | Como jefe de seguridad, quiero filtrar el historial por fecha y tipo de evento (Merma, Acceso, Desconexión) para agilizar las investigaciones. | **Given** una bitácora con múltiples registros, **When** el usuario selecciona el filtro "Solo Discrepancias", **Then** el sistema oculta los accesos normales.<br>**Given** la selección de un rango de fechas, **When** aplica el filtro, **Then** muestra solo los eventos de ese lapso. | EP04 |
...
| **TS18** | Generación de Tokens JWT | Como developer, quiero implementar JWT en la API para mantener sesiones seguras y escalables sin sobrecargar la base de datos. | **Given** un login exitoso, **When** la API responde, **Then** adjunta un token Bearer firmado con expiración.<br>**Given** una petición a ruta protegida con token expirado, **When** la API recibe el request, **Then** responde HTTP 401. | EP05, EP09 |
| **US19** | Vincular Sensor IoT a Zona | Como instalador/jefe de seguridad, quiero ingresar el MAC Address del ESP32 en el sistema para asociarlo a una puerta específica. | **Given** el usuario ingresa la MAC y la Zona, **When** da clic en vincular, **Then** NodeSecure registra el nodo y espera su primer "Ping".<br>**Given** una MAC ya registrada en otra cuenta, **When** se intenta vincular, **Then** el sistema bloquea por seguridad. | EP08 |

### Relación de Epics y User Stories – NodeSecure

* **Epic 01: Monitoreo y Dashboard IoT:** US01, US02, US03, US12.
* **Epic 02: Gestión de Infraestructura Logística:** US04, US05, US13, US17.
* **Epic 03: Alertas y Motor de Discrepancias:** US03, US05, US06, US07.
* **Epic 04: Auditoría y Trazabilidad:** US07, US08, US10, US11.
* **Epic 05: Integraciones API y Backend:** TS09, TS18.
* **Epic 06: Landing Page y Adquisición:** US14, US15.
* **Epic 07: Modelo SaaS y Facturación:** US15, US16.
* **Epic 08: Integración de Hardware (Sensores):** US02, TS09, US19.
* **Epic 09: Identidad y Control de Accesos (IAM):** US13, US17, TS18.
* **Epic 10: Experiencia de Usuario Móvil:** US12.

## 3.2. Impact Mapping.

El Impact Mapping nos permite visualizar la relación directa entre nuestras metas de negocio (Business Goals), los actores involucrados (User Personas), los cambios de comportamiento deseados (Impacts) y las funcionalidades del software que construirán ese puente (Deliverables/User Stories).

**Segmento 1: Dueños y Administradores de PYMES**  
*(Insertar imagen del Impact Map para el Segmento 1)*  
![Impact Mapping - Segmento 1](images/impact-mapping-seg1.png)

**Segmento 2: Jefes de Seguridad y Operaciones**  
*(Insertar imagen del Impact Map para el Segmento 2)*  
![Impact Mapping - Segmento 2](images/impact-mapping-seg2.png)

## 3.3. Product Backlog.

A continuación, se presenta el Product Backlog inicial de NodeSecure, priorizado con base en el valor de negocio y estimado mediante Story Points (Sucesión de Fibonacci: 1, 2, 3, 5, 8). Las prioridades altas (Sprint 1 y 2) se centran en la Landing Page, la arquitectura base y la autenticación.
...
| 5 | US01 | Visualizar red de almacenes | Como dueño de PYME, quiero visualizar mis almacenes registrados para tener un control general. | 3 |
| 6 | US04 | Registrar nueva infraestructura | Como dueño de PYME, quiero registrar un nuevo almacén y sus zonas para empezar a monitorearlos. | 5 |
| 7 | US19 | Vincular Sensor IoT a Zona | Como jefe de seguridad, quiero ingresar el MAC Address del ESP32 para asociarlo a una puerta. | 5 |
| 8 | TS09 | Recepción de payloads IoT (API)| Como developer, quiero exponer un endpoint para recibir los eventos del ESP32. | 8 |
| 9 | US03 | Auditoría de discrepancias (R-T)| Como jefe de operaciones, quiero monitorear la detección de mermas en tiempo real. | 8 |
| 10 | US08 | Bitácora de trazabilidad | Como auditor, quiero consultar un historial detallado de eventos físicos para auditorías de CCTV. | 5 |
| 11 | US10 | Filtros avanzados de auditoría | Como jefe de seguridad, quiero filtrar el historial por fecha y tipo de evento. | 3 |
| 12 | US06 | Notificaciones Push de seguridad| Como jefe de seguridad, quiero recibir notificaciones Push ante aperturas no autorizadas. | 5 |
| 13 | US16 | Suscripción y Facturación | Como dueño de PYME, quiero registrar mi método de pago para activar mi plan y límites. | 8 |
| 14 | US11 | Exportar reporte de incidentes | Como administrador, quiero descargar un reporte PDF/Excel mensual de mermas y alertas. | 3 |
| 15 | US07 | Justificar falsas alarmas | Como jefe de operaciones, quiero poder marcar una alerta como "Falsa Alarma" con justificación. | 3 |
| 16 | US17 | Gestión de Roles (RBAC) | Como dueño de PYME, quiero asignar roles (Administrador, Almacenero, Auditor). | 5 |
...
En esta sección se abarca el planteamiento integral de la propuesta de diseño y arquitectura de **NodeSecure**. Tomando como base los requisitos funcionales extraídos del Product Backlog y el análisis de dominio (Domain-Driven Design), este capítulo define los lineamientos visuales (Style Guidelines), la estructuración de la navegación (Information Architecture) y las interfaces de usuario (UX/UI Design) orientadas específicamente al entorno logístico B2B. El objetivo es garantizar que tanto la Landing Page como el Dashboard IoT ofrezcan una experiencia fluida, accesible y preparada para la operatividad en tiempo real bajo escenarios críticos de seguridad.
...
**Logo de NodeSecure**
El imagotipo de NodeSecure integra visualmente conceptos fundamentales del sector de la seguridad logística: protección, interconectividad y monitoreo. El diseño emplea una forma de escudo estilizado que se fusiona con nodos de red, representando los dispositivos IoT (ESP32 y sensores magnéticos) comunicándose con la plataforma central. 

**Tipografía**
Para el desarrollo de la Landing Page y la Web Application de NodeSecure, se ha seleccionado la tipografía **Inter**. Al ser una plataforma enfocada en la auditoría de inventarios y visualización de discrepancias en tiempo real, la legibilidad es crítica. Inter es una fuente Sans-Serif diseñada para pantallas de alta resolución que facilita la lectura rápida de tablas, alertas críticas y *timestamps*, reduciendo la fatiga visual de los operarios y auditores.

* **Regular (400):** Para cuerpo de texto y descripciones de eventos en la bitácora.
* **Medium (500):** Para botones, inputs y navegación.
* **Semi-Bold (600) / Bold (700):** Para títulos (H1, H2), métricas clave del dashboard y alertas críticas.

**Paleta de Colores**
La paleta de NodeSecure transmite autoridad, seguridad y alerta temprana:
* **Azul Marino (Primary - #0F172A):** Transmite solidez corporativa y confianza para la gestión B2B.
* **Cian Eléctrico (Accent - #06B6D4):** Representa la tecnología IoT y el flujo de datos.
* **Rojo Alerta (Danger - #EF4444):** Color semántico crítico, utilizado exclusivamente para "Discrepancias Físicas" y salidas de stock no autorizadas.
* **Verde Éxito (Success - #22C55E):** Indica sensores "Online" y movimientos de stock conciliados.
* **Gris Pizarra (Background - #1E293B):** Base para el Dark Mode, ideal para largas jornadas de monitoreo en pantallas de seguridad.
...
NodeSecure emplea un modelo de organización **Jerárquico y Visual**. En la capa superior (Dashboard), el gerente visualiza el estado macro de sus almacenes y la cantidad de sensores conectados. En niveles inferiores, la información se detalla en historiales cronológicos (Bitácora de Trazabilidad) y gestión matricial (Usuarios y Permisos). Los procesos de configuración (como vincular un nuevo ESP32) utilizan una organización secuencial guiada para evitar errores de hardware.

### 4.2.2. Labeling Systems
Las etiquetas se han diseñado para evitar jerga técnica y hablar el lenguaje de la logística ("Ubiquitous Language"):
* *Dashboard* (Resumen operativo)
* *Mis Almacenes* (Gestión de sucursales)
* *Dispositivos IoT* (En lugar de "Hardware" o "Sensores MQTT")
* *Trazabilidad* (Bitácora de eventos y discrepancias)
* *Equipo y Accesos* (Gestión de roles)

### 4.2.3. SEO Tags and Meta Tags

**Para la Landing Page:**
* **Title:** NodeSecure | Seguridad Inteligente e IoT para Control de Inventarios
* **Meta Description:** Plataforma web que elimina la merma y el robo hormiga cruzando datos de inventario con sensores IoT en tiempo real. Protege tu almacén hoy.
* **Keywords:** control de inventarios, seguridad IoT, robo hormiga, software para almacenes, ESP32 logística, auditoría CCTV.

### 4.2.4. Searching Systems
El sistema de búsqueda en la Web Application permite al jefe de operaciones filtrar grandes volúmenes de datos. Los filtros principales en la "Bitácora" incluyen: **Búsqueda por fecha (Timestamps)**, **Filtro por tipo de alerta** (Discrepancias vs. Accesos normales) y **Búsqueda por ID de Sucursal/Zona**.

### 4.2.5. Navigation Systems
La navegación global del Landing Page es una barra superior (Navbar) con scroll fluido hacia las secciones clave. Para la Web Application, se utiliza un **Sidebar persistente** en Desktop (que agrupa los módulos principales) y una **Bottom Navigation Bar** en Mobile, permitiendo a los almaceneros operar el sistema a una sola mano mientras realizan tareas físicas.

## 4.3. Landing Page UI Design

### 4.3.1. Landing Page Wireframe
El wireframe de la Landing Page de NodeSecure traza un recorrido de conversión claro (AIDA: Atención, Interés, Deseo, Acción):
* **Hero Section:** Título de alto impacto sobre la eliminación del "robo hormiga", acompañado de un botón de "Prueba Gratuita" y una ilustración conceptual de un almacén conectado.
* **Features:** Disposición en Grid (3 columnas) detallando: 1) Detección de Discrepancias, 2) Integración IoT, 3) Timestamps para CCTV.
* **Pricing:** Tarjetas de planes SaaS (Básico, Premium, Corporativo).
* **About Us / Team:** Tarjetas con las fotos y perfiles del equipo SafeZone.
* **Footer:** Formulario de contacto B2B y enlaces a documentación.

*(Aquí debes insertar las imágenes de tus wireframes de la Landing Page de NodeSecure)*

### 4.3.2. Landing Page Mock-up
El diseño de alta fidelidad (Mock-up) aplica las pautas de estilo Dark Mode. El azul marino domina el fondo, permitiendo que los botones de *Call to Action* en color cian destaquen significativamente. Se emplean mockups 3D de paneles web e íconos minimalistas para ilustrar la facilidad de conectar sensores a puertas físicas.

*(Aquí debes insertar las imágenes de tus mock-ups finales de la Landing Page de NodeSecure)*
